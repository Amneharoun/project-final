...
const Pharmacie = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        await api.put(`/pharmacies/${formData._id}`, dataToSend);
  ...
  // Définir les coordonnées manuellement
  ...
  // Récupérer position automatique
  const getGeolocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        alert(
          `📍 Coordonnées récupérées : ${latitude.toFixed(
            6
          )}, ${longitude.toFixed(6)}`
        );
      },
  ...
  const searchNearbyPharmacies = async () => {
    ...
    setMapLoading(true);
    try {
      const response = await api.get("/pharmacies/proches", {
        params: {
  ...
  const PharmacyMap = ({ pharmacies, patientPos, selectedPharma }) => {
    ...
    useEffect(() => {
      ...
      mapInstance.current = L.map(mapRef.current).setView(
        patientPos
          ? [patientPos.latitude, patientPos.longitude]
          : [13.8373, 20.8312],
        13
      );
      ...
      pharmacies.forEach((pharma) => {
        if (pharma.localisation?.coordinates) {
          ...
          L.marker([lat, lng]).addTo(mapInstance.current).bindPopup(`
              <div>
                <h6><strong>${pharma.nom}</strong></h6>
                <p>${pharma.adresse}</p>
                <p>📞 ${pharma.telephone || "N/A"}</p>
                <button onclick="window.open('https://www.google.com/maps/dir/${
                  patientPos.latitude
                },${patientPos.longitude}/${lat},${lng}', '_blank')" 
  ...
  const toggleStatut = async (id) => {
    try {
      await api.patch(`/pharmacies/${id}/statut`, {});
      fetchPharmacies();
    } catch (error) {
  ...
  const handleEdit = (pharmacie) => {
    setFormData({
      ...
      statutOuvert:
        pharmacie.statutOuvert !== undefined ? pharmacie.statutOuvert : true,
    });
                    ...
                    <tr key={pharma._id}>
                      ...
                      <td>
                        {pharma.localisation?.coordinates
                          ? `${
                              pharma.localisation.coordinates[1]?.toFixed(4) ||
                              0
                            }, ${
                              pharma.localisation.coordinates[0]?.toFixed(4) ||
                              0
                            }`
                          : "Non définie"}
                      </td>
                      <td>
                        <span
                          className={`badge ${
                            pharma.statutOuvert ? "bg-success" : "bg-danger"
                          }`}
                        >
                      ...
                      <div className="modal-body">
                        ...
                        <div className="row">
                          <div className="col-md-6 mb-3">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleHoraireChange("ouverture", e.target.value)
                              }
                            />
                          </div>
                          <div className="col-md-6 mb-3">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleHoraireChange("fermeture", e.target.value)
                              }
                            />
                          </div>
                        </div>
                        ...
                        <div className="mb-3 p-3 border rounded">
                          ...
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            ...
                            >
                              {localisation.coordinates[0] !== 0
                                ? `Coordonnées définies: ${localisation.coordinates[1].toFixed(
                                    6
                                  )}, ${localisation.coordinates[0].toFixed(6)}`
                          ...
                          <small className="text-muted mt-2 d-block">
                            Format: [Longitude, Latitude]. Exemple: [-4.000000,
                            5.000000]
                          </small>
                        </div>
            ...
            <div className="col-md-4">
                ...
                <div className="card-body">
                  ...
                  <div className="mb-3">
                    ...
                    {!patientPosition ? (
                      ...
                      <div className="text-success">
                        ...
                        <br />
                        <small>
                          Lat: {patientPosition.latitude.toFixed(6)}, Lng:{" "}
                          {patientPosition.longitude.toFixed(6)}
                        </small>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pharmacies-list">
                    <h6 className="border-bottom pb-2">
                      Pharmacies trouvées: {nearbyPharmacies.length}
                    </h6>
              ...
              {selectedPharmacy && (
                <div className="card">
                  ...
                  <div className="card-body">
                    <p>
                      <strong>Adresse:</strong> {selectedPharmacy.adresse}
                    </p>
                    <p>
                      <strong>Téléphone:</strong>{" "}
                      {selectedPharmacy.telephone || "N/A"}
                    </p>
                    <p>
                      <strong>Horaires:</strong>{" "}
                      {selectedPharmacy.horaires?.ouverture || "N/A"} -{" "}
                      {selectedPharmacy.horaires?.fermeture || "N/A"}
                    </p>
                    <p>
                      <strong>Statut:</strong>
                      <span
                        className={`badge ${
                          selectedPharmacy.statutOuvert
                            ? "bg-success"
                            : "bg-danger"
                        } ms-2`}
                      >
                    ...
                    <div className="d-grid gap-2">
                      <button
                        ...
                        onClick={() => {
                          const [lng, lat] =
                            selectedPharmacy.localisation.coordinates;
                          window.open(
                            `https://www.google.com/maps/dir/${patientPosition.latitude},${patientPosition.longitude}/${lat},${lng}`,
                            "_blank"
                          );
                        }}
                      >
